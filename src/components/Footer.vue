<template>
    <div class="my-12 px-4 text-caption" style="opacity: .8;
  z-index: 1;">
...
export default {
  ...
  mounted() {
    ...
    function setTime() {
      //mouth要少一个月不然会出问题
      const create_time = Math.round(new Date(Date.UTC(2022, 2, 11, 0, 0, 0)).getTime() / 1000);
      ...
      if(document.getElementById("htmer_time")!=null)
        document.getElementById("htmer_time").innerHTML = "EtherealCraft服务器已经运行"+currentTimeHtml;
    }
    //即时刷新
